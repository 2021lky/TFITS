...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_path", help="path of dataset file", type=str, default='./origin/PEMS/PEMS04.npz')
    ...
    parser.add_argument("--dataset_name", help="生成数据集的名称", type=str, default="PEMS04")
    parser.add_argument(
    ...
    data = data['data'][:, :, 0]
    length = len(data) // args.seq_len
    train_ratio = 0.7
    valid_ratio = 0.15
    train_data = data[:int(train_ratio * length)*args.seq_len]
    valid_data = data[int(train_ratio * length)*args.seq_len: int((train_ratio + valid_ratio) * length)*args.seq_len]
    test_data = data[int((train_ratio + valid_ratio) * length)*args.seq_len:]


    # 标准化
    scaler = StandardScaler()
    train_set_X = scaler.fit_transform(train_data)
    val_set_X = scaler.transform(valid_data)
    test_set_X = scaler.transform(test_data)
